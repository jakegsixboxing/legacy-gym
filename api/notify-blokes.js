...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Any authenticated user (member or staff) can trigger this — recipients are always Local Blokes-only,
    // enforced by the get_blokes_push_subscriptions() RPC (security definer, filters to is_blokes internally,
    // and excludes the caller's own subscriptions so a Blokes member doesn't get pinged on their own message).
    const subs = await fetch(SB_URL + "/rest/v1/rpc/get_blokes_push_subscriptions", {
