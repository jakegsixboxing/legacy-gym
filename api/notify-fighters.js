...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Any authenticated user (member or staff) can trigger this — recipients are always Sparring Club
    // fighters-only, enforced by the get_fighter_push_subscriptions() RPC (security definer, filters to
    // is_fighter internally, and excludes the caller's own subscriptions so a fighter doesn't get pinged
    // on their own message).
    const subs = await fetch(SB_URL + "/rest/v1/rpc/get_fighter_push_subscriptions", {
